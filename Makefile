...
isort_check: ## check that isort has been run
	isort --check-only edx_operation/

isort: ## run isort to sort imports in all Python files
	isort --atomic edx_operation/
...
dummy_translations: ## generate dummy translation (.po) files
	cd edx_operation && i18n_tool dummy
...
open-devstack: ## open a shell on the server started by start-devstack
	docker exec -it edx-operation /edx/app/edx-operation/devstack.sh open

pkg-devstack: ## build the edx-operation image from the latest configuration and code
	docker build -t edx-operation:latest -f docker/build/edx-operation/Dockerfile git://github.com/openedx/configuration

detect_changed_source_translations: ## check if translation files are up-to-date
	cd edx_operation && i18n_tool changed

validate_translations: fake_translations detect_changed_source_translations ## install fake translations and check if translation files are up-to-date

docker_build:
	docker build . -f Dockerfile -t openedx/edx-operation
...
%-attach:
	docker attach edx_operation.$*

github_docker_build:
	docker build . -f Dockerfile --target app -t openedx/edx-operation

github_docker_tag: github_docker_build
	docker tag openedx/edx-operation openedx/edx-operation:${GITHUB_SHA}

github_docker_auth:
	echo "$$DOCKERHUB_PASSWORD" | docker login -u "$$DOCKERHUB_USERNAME" --password-stdin

github_docker_push: github_docker_tag github_docker_auth ## push to docker hub
	docker push 'openedx/edx-operation:latest'
	docker push "openedx/edx-operation:${GITHUB_SHA}"
